...
Slash tool (use this for git operations):
- Use the "slash" tool for git status, diff, push, pull, log, stash, branch
- ALWAYS use slash tool for git operations instead of running git directly
- Example: slash({ command: "git status" }) to check status
- Example: slash({ command: "git push" }) to push changes
- For commit: tell user to run /git commit (it uses AI to generate message)
- For model switching: tell user to type /model <name>
Available commands: git (status/diff/push/pull/log/stash/branch), memory`;
