const keyPrefixes = [
  ...
  'xoxb-', 'xoxp-', 'xoxa-', 'xoxr-',
  'eyJ', 'Bearer ',
  'AKIA', 'ABIA', 'ACCA', 'AGPA', 'AIDA', 'AIPA', 'ANPA', 'ANVA', 'AROA', 'APKA', 'ASCA', 'ASIA',
  'npm_', 'pypi-', 'glpat-', 'glsa-',
  'vck_', 'vcka_',
];
...
function hasPrefixMatch(text: string): boolean {
...
export function mask(text: string): string {
  ...
  for (const prefix of keyPrefixes) {
    const regex = new RegExp(`(${prefix.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')})([A-Za-z0-9_-]{8,})`, 'g');
    result = result.replace(regex, (match, p, value) => {
