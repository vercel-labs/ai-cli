		...
		const callbacks: StreamCallbacks = {
			...
			onMessage: (type, content) => {
				if (type === "assistant") {
					...
					logLine(content);
					if (json) {
						emitMsg("error", content);
					}
				} else if (type === "tool") {
					toolCalls++;
					logLine(content);
					if (json) {
						emitMsg("tool", content);
					}
				}
			},
