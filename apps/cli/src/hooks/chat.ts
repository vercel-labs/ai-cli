...
function addHistoryCacheBreakpoint(
	...
	if (!isAnthropicModel(model) || history.length === 0) {
		return;
	}
...
- Do NOT try to open a browser or verify visually. Verify by reading files instead.`;
	...
	if (silent) {
		...
		Promise.resolve(result.usage)
			.then((u) => {
				if (u?.totalTokens) {
					callbacks.onTokens((t) => t + (u.totalTokens ?? 0));
				}
				if (u) {
					callbacks.onUsage(extractTokenUsage(u as UsageResult));
				}
			})
		...
		try {
			for await (const part of contResult.fullStream) {
				if (part.type === "text-delta") {
					...
					callbacks.onStatus(
						part.text.replaceAll(/\s+/g, " ").trim().slice(-80),
					);
				}
			}
		} catch (error) {
