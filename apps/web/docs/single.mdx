...
`filter`, `rank`, and `pick` read finite record collections through EOF.
They accept lines, JSON arrays, and JSONL, and print the selected records
directly in both terminals and pipes. JSON input produces a JSON array;
lines and JSONL retain selected line contents. `--json` emits inline decision
metadata and original records without writing files.
...
See [Filter, Rank & Pick](/docs/decisions) for uncertainty handling and format flags.
...
The `AI_CLI_OUTPUT_DIR` environment variable sets a default directory for generated artifacts. Decision commands always use stdout and do not use this setting:
