...
Keep records matching a natural-language criterion.

```bash
git log --oneline | ai filter "describes a concurrency fix"
ai filter "describes a bug" --on-uncertain skip < issues.json
```

## rank

Score records against a shared rubric and return them highest first.

```bash
ai rank "impact on signing in" --top 5 < issues.json
ai rank "severity" --rubric severity.json < issues.jsonl
```

## pick

Select one existing record, with separate no-match and uncertain outcomes.

```bash
ai pick "most relevant to this failure" --context failure.log < issues.jsonl
```

All three accept `--model`, `--input auto|lines|json|jsonl`, `--context`,
`--concurrency`, `--timeout`, `--quiet`, and `--json`. `filter` and `pick`
accept `--threshold` (default `0.8`). `filter` also accepts
`--on-uncertain error|skip|keep` (default `error`). `rank` accepts `--top`;
`rank` and `pick` accept `--rubric`.

See [Filter, Rank & Pick](/docs/decisions) for input formats, complete flag
semantics, probability thresholds, custom rubrics, batching, and metadata output.
