...
<Properties>
  ...
  <Property name="AI_CLI_CACHE" type="0 | 1">
    `1` enables prompt caching for all generation commands.
  </Property>
  <Property name="AI_CLI_CACHE_DIR" type="path">
    Cache directory. Defaults to `XDG_CACHE_HOME/ai-cli` or `~/.cache/ai-cli`.
  </Property>
  <Property name="AI_CLI_CACHE_TTL" type="number">
    Cache TTL in seconds. Defaults to `604800` (7 days).
  </Property>
...
- `--no-cache` takes priority over `AI_CLI_CACHE`.
- `--cache-ttl` takes priority over `AI_CLI_CACHE_TTL`.
